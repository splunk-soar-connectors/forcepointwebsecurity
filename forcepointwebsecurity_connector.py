...
class ForcepointWebSecurityConnector(BaseConnector):
    ...
    def _process_json_response(self, r, action_result):
        ...
        try:
            ...
            return action_result.set_status(phantom.APP_ERROR, f"Unable to parse JSON response. Error: {e!s}"), None
    ...
    def _process_text_response(self, r, action_result):
        ...
        try:
            ...
            return action_result.set_status(phantom.APP_ERROR, f"Unable to parse text response as JSON. Error: {e!s}"), None
    ...
    def _process_response(self, r, action_result):
        ...
        message = "Can't process response from server. Status Code: {} Content-Type: {} Data from server: {}".format(
    ...
    def _make_rest_call(self, endpoint, action_result, headers=None, params=None, data=None, method="get"):
        ...
        url = f"https://{self._base_url}:{self._port}/api/web/v1/{endpoint}"
        ...
        try:
            ...
            return action_result.set_status(phantom.APP_ERROR, f"Error Connecting to server. Details: {e!s}"), resp_json
    ...
    def _start_transaction(self, action_result):
        ...
        if phantom.is_fail(ret_val):
            message = f"Failed to start transaction. Error Response: {response}. Error Message: {action_result.get_message()}"
            return action_result.set_status(phantom.APP_ERROR, message), None
        ...
        message = f"Missing Transaction ID when starting transaction: {response}"
        return action_result.set_status(phantom.APP_ERROR, message), None
    ...
    def _commit_transaction(self, action_result, transaction_id):
        ...
        if phantom.is_fail(ret_val):
            message = f"Failed to commit transaction. Error Response: {response}. Error Message: {action_result.get_message()}"
            return action_result.set_status(phantom.APP_ERROR, message), None
        ...
        message = f'Missing "Commit Time" when attempting to commit transaction: {response}'
        return action_result.set_status(phantom.APP_ERROR, message), None
    ...
    def _rollback_transaction(self, action_result, transaction_id):
        ...
        if phantom.is_fail(ret_val):
            message = f"Failed to rollback transaction. Error Response: {response}. Error Message: {action_result.get_message()}"
        ...
        self.append_to_message(f'Missing "Rollback Time" when attempting to cancel transaction: {response}')
        return None
    ...
    def _get_categories(self, action_result):
        ...
        if phantom.is_fail(ret_val):
            ...
            message += f" Error Response: {response}. Error Message: {action_result.get_message()}"
            return action_result.set_status(phantom.APP_ERROR, message), None
        ...
        message = f'Missing "Categories" from the response while trying to get the list of categories: {response}'
        return action_result.set_status(phantom.APP_ERROR, message), None
    ...
    def _create_category(self, action_result, transaction_id, category_name, description=None, parent_id=0):
        ...
        data = {"Transaction ID": transaction_id, "Categories": [{"Category Name": category_name, "Parent": parent_id}]}
        ...
        if phantom.is_fail(ret_val):
            message = f"Failed to create category. Error Response: {response}. Error Message: {action_result.get_message()}"
            return action_result.set_status(phantom.APP_ERROR, message), None
    ...
    def _handle_test_connectivity(self, param):
        ...
        if phantom.is_fail(ret_val):
            ...
            self.save_progress(f"Test connectivity failed: {response}")
    ...
    def _handle_block_objects(self, param):
        ...
        if cat.lower() not in [category["Category Name"].lower() for category in self._flatten_categories(categories["Categories"])]:
            if not create_cat:
                ...
                message = (
                    f'Category "{cat}" does not exist. Create the category or enable the "create_category" option on this action and try again'
                )
            ...
            for url in [url.strip() for url in param["url"].split(",")]:
                if "://" in url:
                    ...
                    for protocol in ["http", "https", "ftp"]:
                        payload["URLs"].append(f"{protocol}://{url}")
    ...
    def _handle_lookup_object(self, param):
        ...
        if "ip" in param:
            ...
            message = f"Missing required parameters (IP or URL): {param}"
            return action_result.set_status(phantom.APP_ERROR, message)
        ...
        message = f"Retrieved categories for each object: {object_list}"
        return action_result.set_status(phantom.APP_SUCCESS, message)
    ...
    def _handle_get_category(self, param):
        ...
        message = f"Retrieved contents of category: {cat}"
        return action_result.set_status(phantom.APP_SUCCESS, message)
    ...
    def _handle_delete_category(self, param):
        ...
        payload = {"Category Names": [cat], "Transaction ID": transaction_id}
        ...
        message = f"Category deleted: {cat}"
        return action_result.set_status(phantom.APP_SUCCESS, message)
